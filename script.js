$(document).ready(function() {
    ...
    $('.timeline-marker').hover(
        function() {
            $(this).closest('.timeline-item').find('.timeline-content').addClass('highlight');
        },
        function() {
            $(this).closest('.timeline-item').find('.timeline-content').removeClass('highlight');
        }
    );
    ...
    // Removed project card hover animation for year button (per user request)
    ...
    // Initialize contact info as hidden
    $('.contact-info').css({
        'opacity': '0',
        'transform': 'translateY(30px)',
        ...
        projects.forEach(function(project) {
            const projectCard = `
                <div class="project-card" data-project-id="${project.id}">
                    <!-- Image section commented out - can be restored later
                    <div class="project-card-image">
                        <img src="${project.image}" alt="${project.title}" onerror="this.src='img/projects/emr-console-thumb.png'">
    ...
    $(document).on('keydown', function(e) {
        if (e.key === 'Escape' || e.keyCode === 27) {
            if ($('#projectModal').hasClass('active')) {
                closeProjectModal();
            }
            if ($('#educationModal').hasClass('active')) {
                closeEducationModal();
            }
        }
    });
    ...
    $('.degree-item').on('click', function() {
        const degreeKey = $(this).data('degree');
        showEducationModal(degreeKey);
    });
